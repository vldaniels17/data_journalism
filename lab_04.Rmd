...
```{r}
mingo_working <-mingo %>%
  mutate(distributer_name = reporter_name)
...
```{r}
# kept getting an error message, I really don't understand what's wrong with it. I commented out to be able to knit
# mingo_working <- mingo %>%
...
# mingo %>%
 ...
 # summarise(shipments = n(), %>%
          #  total_pills = sum(dosage_unit))
  # im pretty stumped on this one, I've tried a bunch of different combos and I don't really know what I'm doing incorrectly 
...
1. I would try to mutate the data to create a new column that contains everything with cardinal in it.I would then rename the column to all_of_cardinal. I would then use a case when function to sort out specifics such as shipment size or whatever else we are trying to look at.
