...
#already installed install.packages("tidycensus") 
#already installed install.packages("mapview")
#already installed install.packages("ggthemes")
#already installed install.packages("scales")
library(tidyverse)
library(janitor)
library(arcos)
library(tidycensus)
...
```{r}
# Comment - the not in pop table has puerto rico included but that won't really be an issues. The not in pills table has a lot of data from a lot of larger states but doesn't include the dosage unit which could cause issues if the population is accounted for but not the pills.
...
geo_pills_population_state_2009 %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills Per Person',title="Tennessee, Kentucky, West Virgina, Oklahoma and Nevada have the highest rates of pills per person", subtitle = "2009 population, U.S. Census", caption = "Source: U.S. Census ACS") +
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Pills Per Person Increased from 2006 to 2012", subtitle = "Kentucky, Tennessee, Nevada, Oklahoma, and West Virgina's rates rose each year", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
#i see two trends, either the number of pills per person are in a similar range for counties close together or there is one particular county in the cluster that has an extremely large amount. I noticed this throughout NC, SC and GA. I also noticed that the counties that make up the larger cities/capital cities have extremely high numbers compared to surrounding counties. 
