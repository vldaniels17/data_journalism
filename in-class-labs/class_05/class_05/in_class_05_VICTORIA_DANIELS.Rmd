...
buyer_annuals <- read_csv ("data/buyer_annual_by_year.csv")
...
```{r}
#buyer_annual higher than buyer_totals - Showing us the annual total by year breaks up buyer totals, which means its splitting those big chunks into smaller pieces which means more parts which means more records

#buyer_addresses higher than buyer_totals - addresses gives us all pharmacies even if they did not have opoid shipments 
...
#va consolidated mail outpatient pharmacy    charlestown, SC   its a Veterans affairs outpatient facility 
```
